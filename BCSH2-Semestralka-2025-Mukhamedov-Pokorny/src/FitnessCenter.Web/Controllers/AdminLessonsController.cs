...
public sealed class AdminLessonsController : Controller
{
    ...
    // ======= KALENDÁŘ LEKCÍ + ADMIN AKCE =======
    public async Task<IActionResult> Calendar(int? year, int? month)
    {
        var today = DateTime.Today;

        // ošetření vstupu: když nic nepřijde, vezmi dnešní měsíc
        int y = (year is > 0) ? year.Value : today.Year;
        int m = (month is >= 1 and <= 12) ? month.Value : today.Month;

        var firstDay = new DateTime(y, m, 1);
        var nextMonth = firstDay.AddMonths(1); // POZOR: používáme < nextMonth
